...
const playersCollection = firestore_1.firestore.collection("players");
const gameRoomsCollection = firestore_1.firestore.collection("gamerooms");
const app = express();
app.use(cors());
...
app.patch("/player-guest", async (req, res) => {
    ...
    const gameRoomRef = firestore_1.rtdb.ref(`gamerooms/${rtdbGameRoomId}/currentGame/playerTwo`);
...
app.post("/game-rooms", async (req, res) => {
    ...
    if (document.exists) {
        const gameRoomRef = firestore_1.rtdb.ref("gamerooms/" + (0, nanoid_1.nanoid)());
...
app.patch("/start", async (req, res) => {
    ...
    const gameRoomRef = firestore_1.rtdb.ref(`gamerooms/${rtdbRoomId}/currentGame/${player}`);
...
app.get("/choice", async (req, res) => {
    const { player, rtdbRoomId } = req.body;
    const playerChoiceRef = firestore_1.rtdb.ref(`gamerooms/${rtdbRoomId}/currentGame/${player}`);
...
app.patch("/replay", async (req, res) => {
    ...
    const playerOneReseting = firestore_1.rtdb.ref(`gamerooms/${rtdbRoomId}/currentGame/playerOne`);
    const playerTwoReseting = firestore_1.rtdb.ref(`gamerooms/${rtdbRoomId}/currentGame/playerTwo`);
    const replayRef = firestore_1.rtdb.ref(`gamerooms/${rtdbRoomId}/currentGame`);
...
app.patch("/grow-score", async (req, res) => {
    try {
        ...
        const ref = firestore_1.rtdb.ref(`gamerooms/${rtdbRoomId}/currentGame/${player}`);
